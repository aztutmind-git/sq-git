...
# ---------- admin: search questions with real filters — never dumps the whole bank ----------
...
def admin_list_questions(
    ...
    if grade:
        q = q.filter(models.Question.grade == grade)
    if level is not None:
        q = q.filter(models.Question.level == level)
    if question_id is not None:
        q = q.filter(models.Question.id == question_id)
    if status:
        q = q.filter(models.Question.status == status)
    if topic:
        q = q.filter(models.Question.topic.ilike(f"%{topic}%"))
    if search:
        q = q.filter(models.Question.question.ilike(f"%{search}%"))
    return q.order_by(models.Question.subject, models.Question.level).limit(min(limit, 200)).all()
...
@router.put("/admin/questions/{question_id}", response_model=schemas.QuestionOut)
def admin_update_question(question_id: int, payload: schemas.QuestionUpdate, db: Session = Depends(get_db),
                           _admin: models.User = Depends(require_admin)):
