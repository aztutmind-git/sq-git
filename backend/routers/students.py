...
def _fresh_progress_rows(db: Session, user_id: str, grade: str) -> List[models.Progress]:
    subjects = get_subjects_for_grade(db, grade)
    return [models.Progress(user_id=user_id, subject=s.key, unlocked_level=1, xp=0, stars={}) for s in subjects]
...
def create_student(payload: schemas.StudentCreate, db: Session = Depends(get_db),
    ...
    for row in _fresh_progress_rows(db, user.id, payload.grade):
        db.add(row)
    db.commit()
    db.refresh(user)
    return user
