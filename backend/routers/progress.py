...
def _to_schema(p: models.Progress) -> schemas.SubjectProgress:
    return schemas.SubjectProgress(
        subject=p.subject, unlocked_level=p.unlocked_level, xp=p.xp,
        stars={str(k): v for k, v in (p.stars or {}).items()},
    )
...
def submit_attempt(payload: schemas.QuizAttemptRequest, db: Session = Depends(get_db),
    ...
    if passed:
        ...
        if payload.level == prog.unlocked_level and prog.unlocked_level < 5:
    ...
    return schemas.QuizAttemptResult(
        passed=passed, stars=stars, xp_gained=xp_gained, progress=_to_schema(prog),
    )
