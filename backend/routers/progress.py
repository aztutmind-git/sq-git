...
def get_performance_dashboard(
    ...
    Progress currently stores stars at level granularity rather than individual
    question attempts. The dashboard therefore uses the best available signal:
    level stars (0-3) are projected onto the questions belonging to that level,
    then rolled up through subject -> chapter -> topic -> stage. This keeps the
    dashboard fully backed by the existing PostgreSQL model without inventing
    ...
    progress_rows = {
        p.subject: p
        for p in db.query(models.Progress).filter(models.Progress.user_id == user.id).all()
    }

    # Subjects mapped to the student's grade, including subjects not enrolled.
    mapped = db.query(models.Subject).join(
        models.GradeSubject, models.GradeSubject.subject_key == models.Subject.key
    ).filter(models.GradeSubject.grade == user.grade).order_by(models.Subject.name).all()
    ...
    for subj in mapped:
        p = progress_rows.get(subj.key)
        enrolled = bool(p and p.enrolled)
        qrows = db.query(models.Question).filter(
            models.Question.subject == subj.key,
            models.Question.status != "draft",
            (models.Question.grade.is_(None) | (models.Question.grade == user.grade)),
        ).all()

        stars = (p.stars or {}) if p else {}
        # Build a stable hierarchy from the question metadata already stored.
        buckets = defaultdict(lambda: {"count": 0, "points": 0.0, "levels": set()})
        for q in qrows:
            chapter = q.chapter or "Unclassified"
            topic = q.topic or "General"
            stage = q.stage or "All stages"
            key = (chapter, topic, stage)
            bucket = buckets[key]
            bucket["count"] += 1
            bucket["levels"].add(q.level)
            level_stars = int(stars.get(str(q.level), 0) or 0)
            bucket["points"] += (level_stars / 3.0) * 100.0

        chapter_map = defaultdict(lambda: {"count": 0, "points": 0.0, "children": defaultdict(lambda: {"count": 0, "points": 0.0, "children": defaultdict(lambda: {"count": 0, "points": 0.0}))}))
        ...
        for chapter, cm in chapter_map.items():
            ...
            for topic, tm in cm["children"].items():
                ...
                for stage, sm in tm["children"].items():
                    ...
                    stages.append({"name": stage, "score": score, "question_count": sm["count"]})
                tscore = round(tm["points"] / tm["count"]) if tm["count"] else 0
                topics.append({"name": topic, "score": tscore, "question_count": tm["count"], "stages": stages})
            cscore = round(cm["points"] / cm["count"]) if cm["count"] else 0
            chapters.append({"name": chapter, "score": cscore, "question_count": cm["count"], "topics": topics})

        subject_count = sum(x["count"] for x in buckets.values())
        subject_points = sum(x["points"] for x in buckets.values())
        score = round(subject_points / subject_count) if subject_count else 0

        # What went well / improvement are descriptive rollups, not judgments.
        scored_chapters = sorted(chapters, key=lambda x: x["score"], reverse=True)
        strong = [x["name"] for x in scored_chapters if x["score"] >= 80][:3]
        improve = [x["name"] for x in sorted(chapters, key=lambda x: x["score"]) if x["score"] < 70][:3]
        ...
        subjects.append({
            ...
            "icon": subj.icon or "📘",
            "enrolled": enrolled,
            "score": score if enrolled else None,
            "xp": p.xp if p else 0,
            "unlocked_level": p.unlocked_level if p else 0,
            "max_level": _max_level_for_subject(db, subj.key),
            "chapters": chapters,
            "what_went_well": strong,
            "improvement": improve,
            "reflection": (
                f"Keep practising {strong[0]}." if strong else
                f"Build consistency across {subj.name}." if enrolled else
                f"{subj.name} is not enrolled."
            ),
        })

        if enrolled and subject_count:
            overall_weight += subject_count
            overall_points += subject_points
    ...
    return {
        "student": {"name": user.name, "grade": user.grade, "board": user.board, "avatar": user.avatar},
