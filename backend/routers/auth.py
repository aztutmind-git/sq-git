...
def create_guest(request: Request, payload: schemas.GuestSignupRequest, db: Session = Depends(get_db)):
    ...
    token). They're enrolled in only the one subject they picked on the
    pre-demo form — same enrollment mechanism a paid student's admin manages —
    and that subject is capped at its demo_level_cap."""
    if payload.subject not in SUBJECT_KEYS:
