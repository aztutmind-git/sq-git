...
from routers.subjects import get_subjects_for_grade
...
def create_guest(request: Request, payload: schemas.GuestSignupRequest, db: Session = Depends(get_db)):
    ...
    token). They get a Progress row for every subject mapped to their grade
    (so those show up as locked cards to preview) but are only enrolled in
    the one subject they picked on the pre-demo form, capped at its
    demo_level_cap."""
    grade_subjects = get_subjects_for_grade(db, payload.grade)
    grade_subject_keys = {s.key for s in grade_subjects}
    if payload.subject not in grade_subject_keys:
        raise HTTPException(status_code=400, detail="That subject isn't offered for the selected grade")
    ...
    for subject in grade_subject_keys:
        db.add(models.Progress(
            user_id=guest.id, subject=subject, unlocked_level=1, xp=0, stars={},
            enrolled=(subject == payload.subject),
        ))
    db.commit()
    db.refresh(guest)
